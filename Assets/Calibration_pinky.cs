...
public class Calibration_pinky : MonoBehaviour
{
    public static Calibration_pinky instance;
    public float threshold_pinky = 0.8f; // 0.8f is default threshold value
    ...
    void Update()
    {
        ...
        if (rightDistance && flagRelaxed && flagEffort){
            ...
            threshold_pinky = Mathf.Abs(distanceInMaxEffortState)*thScalingFactor;
            UnityEngine.Debug.Log("Threshold calibrated!!! " + threshold_pinky);
            flagRelaxed = false;
            flagEffort = false;
    ...
    void takeRelaxedPositionsDistances(){
        ...
        if (rightDistance && (Input.GetKeyDown(KeyCode.Space)) && (!flagRelaxed)) // this part needs adjustment 
        {
            ...
            distanceInRelaxedState = Vector3.Distance(HandTracking.instance.handPoints[4].transform.localPosition, HandTracking.instance.handPoints[16].transform.localPosition);
            flagRelaxed = true;
    ...
    void takeMaxEffortPositionsDistances(){
        ...
        if (rightDistance && (Input.GetKeyDown(KeyCode.Space)) && (flagRelaxed)) // this part needs adjustment
        {
            ...
            distanceInMaxEffortState = Vector3.Distance(HandTracking.instance.handPoints[4].transform.localPosition, HandTracking.instance.handPoints[16].transform.localPosition);
            flagEffort = true;
        } 
