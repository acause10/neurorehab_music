...
public class Calibration_ring : MonoBehaviour
{
    public static Calibration_ring instance;
    public float threshold_ring = 0.8f;
    ...
    void Update()
    {
        ...
        if (rightDistance && flagRelaxed && flagEffort){
            ...
            threshold_ring = Mathf.Abs(distanceInMaxEffortState)*thScalingFactor;
            UnityEngine.Debug.Log("Threshold calibrated!!! " + threshold_ring);
            flagRelaxed = false;
            flagEffort = false;
    ...
    void takeRelaxedPositionsDistances(){
        ...
        if (rightDistance && (Input.GetKeyDown(KeyCode.Space)) && (!flagRelaxed)) // this part needs adjustment 
        {
            ...
            distanceInRelaxedState = Vector3.Distance(HandTracking.instance.handPoints[4].transform.localPosition, HandTracking.instance.handPoints[20].transform.localPosition);
            flagRelaxed = true;
    ...
    void takeMaxEffortPositionsDistances(){
        ...
        if (rightDistance && (Input.GetKeyDown(KeyCode.Space)) && (flagRelaxed)) // this part needs adjustment
        {
            ...
            distanceInMaxEffortState = Vector3.Distance(HandTracking.instance.handPoints[4].transform.localPosition, HandTracking.instance.handPoints[20].transform.localPosition);
            flagEffort = true;
        } 
